...
class ModelConfig:
    ...
    USE_CUDA = torch.cuda.is_available()
    # use_cuda = False
    data_path = 'datasets/BV12i42167pY_BV1rx4y1t7sq.csv'
...
for i_fold_round in range(k_fold):
    ...
    model_config.save_path = f'model/irony_detection_bert_bilst[{i_fold_round + 1}-{k_fold}K_fold].pth'
    train_loader = irony_detection_class.IronyDataLoader_Func(model_config, 'train', i_fold_round, k_fold)
    test_loader = irony_detection_class.IronyDataLoader_Func(model_config, 'test', i_fold_round, k_fold)
...
result_df.to_csv(f'result/irony_detection_{k_fold}_fold_result.csv', index=False)
